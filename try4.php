...
for ($i = 1; $i < count($csv_data); $i++) {
    ...
    // Ensure the row has the same number of values as modified headers
    $row = array_slice($row, 0, count($modified_headers));

    foreach ($modified_headers as $index => $header) {
        $value = $row[$index];
        $insert_data_query .= "'" . $mysqli->real_escape_string($value) . "', ";
    }
    $insert_data_query = rtrim($insert_data_query, ", ") . "), ";
}
$insert_data_query = rtrim($insert_data_query, ", "); // Remove the trailing comma
$insert_data_query .= ";"; // Add this line to close the query with a semicolon
